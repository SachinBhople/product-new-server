...
export const getProductDetails = asyncHandler(async (req: Request, res: Response): Promise<void> => {
    ...
    const result = await Product.findOne({ _id: productDetailId })
    res.json({ message: "Product Details Fetch Success", result });
});
export const getadminAllProducts = asyncHandler(async (req: Request, res: Response): Promise<void> => {
    const result = await Product.find()
    res.json({ message: " Admin Product Fetch Success", result });
});
